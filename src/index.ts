...
import { runContainer, stopContainer, STDOUT_LOG, waitForMarker } from './docker/run'
...
async function runBounded(
  ...
  const winner = await Promise.race([
    ...
    Bun.sleep(plan.timeoutSeconds * 1000).then(() => ({ kind: 'timeout' as const })),
  ])
...
async function runWithScreenshot(
  ...
  const container = runContainer(spec, { logDir, stopTimeoutSeconds: STOP_TIMEOUT_SECONDS })
  const settled = Bun.sleep(plan.renderWaitSeconds * 1000).then(() => 'ready' as const)
...
async function inspectContainer(name: string): Promise<ContainerInfo | null> {
  const proc = Bun.spawn(['docker', 'inspect', name], { stdout: 'pipe', stderr: 'ignore' })
...
async function verify(args: ParsedArgs, config: RootConfig, plugins: Map<string, GamePlugin>): Promise<number> {
  ...
  const mods: BoundMod[] = await Promise.all(
  ...
  const stale = mods.filter((mod) => mod.stale)
  if (args.json) {
    const payload = {
      container: name,
      running: true,
  ...
  process.stdout.write(`${renderVerify(name, info, plan, mods)}\n`)
  return stale.length > 0 ? Exit.Stale : Exit.Ok
}
...
function renderVerify(
  ...
  if (mods.length === 0) {
    out.push('  no mods are bind-mounted into this container')
    return out.join('\n')
  }

  const paths = mods.map((mod) => shortenHome(mod.hostDir))
  const idWidth = Math.max(...mods.map((mod) => mod.packageId.length))
  const pathWidth = Math.max(...paths.map((p) => p.length))
  const stamps = mods.map((mod) =>
