...
/**
 * A path is anything that looks like one; everything else is a package. Native resolution still
 * walks node_modules in a compiled binary, and package.json is always resolvable.
 */
function locate(spec: string, from: string): string {
  ...
  if (expanded.startsWith('.') || isAbsolute(expanded)) {
    ...
    try {
      target = dirname(Bun.resolveSync(`${expanded}/package.json`, from))
    } catch {
